  ...
  if (reviewCard.nextCard) {
    ...
    */
    ...
    loading.value = false;
        ...
        showNotify({ type: 'warning', message: "已经是最后一张了" });
      }
};
  ...
  if (reviewCard.prevCard) {
    ...
    */
    ...
    loading.value = false;
        ...
        showNotify({ type: 'warning', message: "已经是第一张了" });
      }
};
const dragTop = () => {
  // 完成复习
...
const onCardDragMove = (obj: ICardDragObject) => {
  ...
  if (Math.abs(obj.left) > Math.abs(obj.top)) {
    ...
    // 拖动提示
    if (direction == "left") {
          showNotify({ type: 'primary', message: "下一张卡片" });
      ...
      if (reviewCard.nextCard) {
        ...
        bottomShowReviewCard.value = reviewCard.currentCard as ICard;
          ...
          showNotify({ type: 'primary', message: "上一张卡片" });
      // 拖动卡片时, 底部的卡片
      if (reviewCard.prevCard) {
    ...
    if (direction == "top") {
          showNotify({ type: 'success', message: "完成复习" });
        } else {
          showNotify({ type: 'danger', message: "忽略本卡片" });
    }
  }
};
